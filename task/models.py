...
@property
def status(self):
    code = "DONE & DELETE" if self.is_done and self.is_delete else "DONE" if self.is_done else "DELETE"
    return code


@property
def css_class(self):
    cls = "dark" if self.is_done and self.is_delete else "succes" if self.is_done else "danger" if self.is_delete else "warning"
